...
export function TaskDetailView({ task, post, related, comments = [] }: { task: TaskKey; post: SitePost; related: SitePost[]; comments?: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <EditableSiteShell>
      <main style={taskThemeStyle(task)} className={`task-detail task-detail-${task} min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]`}>
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <>
      <article className="task-detail-article mx-auto max-w-5xl px-6 py-14 sm:py-24">
        <BackLink task="article" />
        <div className="mt-10 border-l-4 border-[var(--tk-accent)] pl-6 sm:pl-10">
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[var(--tk-accent)]">{categoryOf(post, 'Article')}</p>
        <h1 className="editable-display mt-5 text-balance text-5xl font-semibold leading-[0.94] tracking-[-0.04em] sm:text-6xl lg:text-7xl">{post.title}</h1>
        <div className="mt-6 text-sm text-[var(--tk-muted)]">
          <span>{SITE_CONFIG.name}</span>
        </div>
        </div>
        {images[0] ? <img src={images[0]} alt="" className="mt-12 aspect-[16/8] w-full border border-[var(--tk-line)] object-cover" /> : null}
...
function ListingDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="listing-detail-layout mx-auto max-w-[var(--editable-container)] px-6 py-14 sm:py-20 lg:px-8">
      <BackLink task="listing" />
      <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1fr)_380px]">
        <article className="min-w-0">
          <div className="listing-identity flex flex-col gap-6 sm:flex-row sm:items-center">
